...
## The amount that the camera's lerp target will move (along x-axis) when a lean is performed.
@export var lean_amnt_pos : float = 0.5
## The amount (in degrees) that the camera will rotate along its z-axis when a lean is performed.
